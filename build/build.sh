...
if [[ ! -f "build/src/${FIREFOX_CHANNEL}-${FIREFOX_LANG}.tar.bz2" ]]; then
    echo "==> Downloading ${FIREFOX_CHANNEL}"
    wget "https://download.mozilla.org/?product=${FIREFOX_CHANNEL}-latest-ssl&os=${FIREFOX_OS}&lang=${FIREFOX_LANG}" -O "build/src/${FIREFOX_CHANNEL}-${FIREFOX_LANG}.tar.bz2"
fi

tar -xvf "build/src/${FIREFOX_CHANNEL}-${FIREFOX_LANG}.tar.bz2" -C build
...
./build/appimagetool -n --comp zstd \
    build/AppDir \
